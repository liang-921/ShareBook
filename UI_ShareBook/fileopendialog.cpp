...
//        path1 = path1.substr(7,path1.size());
        pic.load(QString::fromStdString(path1));
        pics.push_back(pic);
        qDebug()<<"path:"<<QString::fromStdString(path1);
        if(pic.isNull())
            qDebug()<<"图片是空的";
//        std::string s = "/root/"+std::to_string(i)+".png";
//        pic.save(QString::fromStdString(s));
//        i++;
        //将图片进行编码后传输
...
//        qDebug()<<QString::fromStdString(encodePhoto(path.toEncoded().toStdString()));
    }
...
//    Client::getInstance()->send(content.toStdString().data(),content.size());
}
